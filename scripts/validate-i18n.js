 ...
 * @returns {Object} Map of locale to message keys
 */
function loadLocaleMessages() {
  ...
  for (const locale of localeDirs) {
    ...
    try {
      ...
      console.error(
        `✗ Failed to parse ${locale}/messages.json: ${error.message}`,
      );
      process.exit(1);
    }
  }
 ...
 * @returns {Array} Array of potential hardcoded strings with locations
 */
 ...
 * @param {Object} locales - Map of locales to message keys
 * @returns {Array} Array of consistency issues
 */
function validateLocaleConsistency(locales) {
  const issues = [];
